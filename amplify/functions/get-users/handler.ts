import {
    ...
    ListUsersCommand, // Changed from ListUsersInGroupCommand to ListUsersCommand
  } from "@aws-sdk/client-cognito-identity-provider";
  import type { Schema } from '../../data/resource'; // Assuming you have a schema definition
  ...
  const cognitoClient = new CognitoIdentityProviderClient({ region: 'us-east-1' }); // Ensure region is correct for your Cognito User Pools
  const appsyncClient = new AppSyncClient({ region: 'us-east-1' }); // Ensure region is correct for your AppSync APIs
  ...
  *
  * @param {Schema["Users"]["functionHandler"]["event"]} event - Event data passed to the Lambda function (not used in this function). // Updated Schema type to "Users"
  * @param {Schema["Users"]["functionHandler"]["context"]} context - Lambda context object containing information about the execution environment. // Updated Schema type to "Users"
  * @returns {Promise<{ users: string, executionDuration: number }>} - A promise that resolves to an object containing // Changed return type to "users"
  *          a JSON string of all user details and the function execution duration. // Updated description to "all user details"
  */
  export const handler: Schema["Users"]["functionHandler"] = async (event, context) => { // Updated Schema type to "Users" and handler name to "handler" (if you rename the function export, rename here as well)
    const start = performance.now();
    console.log("Starting getUsers Lambda function execution"); // Updated log message to getUsers
    ...
    try {
        ...
        return {
            users: JSON.stringify([]), // Return empty list on error // Changed key to "users"
    ...
    if (!tags) {
        ...
        return {
            users: JSON.stringify([]), // Return empty list if no tags // Changed key to "users"
            executionDuration: performance.now() - start,
        };
    }
    ...
    if (!deploymentType) {
        ...
        return {
            users: JSON.stringify([]), // Return empty list if deployment type is missing // Changed key to "users"
            executionDuration: performance.now() - start,
        };
    }
    ...
    if (deploymentType === "sandbox") {
        ...
        if (!stackName) {
            ...
            return {
                users: JSON.stringify([]), // Return empty list if stack name is missing for sandbox // Changed key to "users"
                executionDuration: performance.now() - start,
            };
        }
        ...
        if (!branchName) {
            ...
            return {
                users: JSON.stringify([]), // Return empty list if branch name is missing for branch // Changed key to "users"
                executionDuration: performance.now() - start,
            };
        }
        ...
        return {
            users: JSON.stringify([]), // Return empty list for unsupported deployment type // Changed key to "users"
            executionDuration: performance.now() - start,
        };
    }
    ...
    try {
        ...
        return {
            users: JSON.stringify([]), // Return empty list on error listing user pools // Changed key to "users"
            executionDuration: performance.now() - start,
        };
    }
    ...
    if (!userPoolsResponse.UserPools || userPoolsResponse.UserPools.length === 0) {
        ...
        return {
            users: JSON.stringify([]), // Return empty list if no user pools are found // Changed key to "users"
            executionDuration: performance.now() - start,
        };
    }
    ...
    if (deploymentType === "sandbox") {
        ...
        try {
            ...
            return {
                users: JSON.stringify([]), // Return empty list on error listing appsync apis // Changed key to "users"
                executionDuration: performance.now() - start,
            };
        }
        ...
        if (!graphqlApisResponse.graphqlApis || graphqlApisResponse.graphqlApis.length === 0) {
            ...
            return {
                users: JSON.stringify([]), // Return empty list if no appsync apis are found // Changed key to "users"
                executionDuration: performance.now() - start,
            };
        }
        ...
        if (!targetAppSync) {
            ...
            return {
                users: JSON.stringify([]), // Return empty list if no matching appsync instance // Changed key to "users"
                executionDuration: performance.now() - start,
            };
        }
        ...
        if (!targetUserPoolId) {
            ...
            return {
                users: JSON.stringify([]), // Return empty list if user pool id is missing in appsync details // Changed key to "users"
                executionDuration: performance.now() - start,
            };
        }
    ...
    if (!targetUserPoolId) {
        ...
        return {
            users: JSON.stringify([]), // Return empty list if no matching user pool is found // Changed key to "users"
            executionDuration: performance.now() - start,
        };
    }
    ...
    // Step 4: List all users from the identified User Pool // Updated Step description
    console.log("Step 4: Listing all users from User Pool:", targetUserPoolId); // Updated log message
    try {
        const listUsersCommand = new ListUsersCommand({ // Changed to ListUsersCommand
        ...
        console.log(`Found ${users.length} users in User Pool ${targetUserPoolId}.`); // Updated log message - removed group specific info
        ...
        // Step 6: Return the list of all users and execution duration // Updated Step description
        console.log("Step 6: Returning user list and execution duration."); // Updated log message
        return {
            users: JSON.stringify(userDetails), // JSON-encoded list of all user details // Changed key to "users" and description
            executionDuration: performance.now() - start,
        };
        ...
        return {
            users: JSON.stringify([]), // Return empty list on error fetching users // Changed key to "users"
            executionDuration: performance.now() - start,
        };
    } finally {
        console.log("getUsers Lambda function execution finished."); // Updated log message to getUsers
    }
  };