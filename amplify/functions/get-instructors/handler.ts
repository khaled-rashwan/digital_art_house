...
/**
* Lambda function handler for retrieving a list of instructors from a Cognito User Pool.
...
*
* @param {Schema["Instructors"]["functionHandler"]["event"]} event - Event data passed to the Lambda function (not used in this function).
* @param {Schema["Instructors"]["functionHandler"]["context"]} context - Lambda context object containing information about the execution environment.
* @returns {Promise<{ instructors: string, executionDuration: number }>} - A promise that resolves to an object containing
*          a JSON string of instructor user details and the function execution duration.
*/
export const handler: Schema["Instructors"]["functionHandler"] = async (event, context) => {
  const start = performance.now();
  console.log("Starting getInstructors Lambda function execution");
  ...
  try {
      ...
      return {
          instructors: JSON.stringify([]), // Return empty list on error
  ...
  if (!tags) {
      ...
      return {
          instructors: JSON.stringify([]), // Return empty list if no tags
          executionDuration: performance.now() - start,
      };
  }
  ...
  if (!deploymentType) {
      ...
      return {
          instructors: JSON.stringify([]), // Return empty list if deployment type is missing
          executionDuration: performance.now() - start,
      };
  }
  ...
  if (deploymentType === "sandbox") {
      ...
      if (!stackName) {
          ...
          return {
              instructors: JSON.stringify([]), // Return empty list if stack name is missing for sandbox
              executionDuration: performance.now() - start,
          };
      }
      ...
      if (!branchName) {
          ...
          return {
              instructors: JSON.stringify([]), // Return empty list if branch name is missing for branch
              executionDuration: performance.now() - start,
          };
      }
      ...
      return {
          instructors: JSON.stringify([]), // Return empty list for unsupported deployment type
          executionDuration: performance.now() - start,
      };
  }
  ...
  try {
      ...
      return {
          instructors: JSON.stringify([]), // Return empty list on error listing user pools
          executionDuration: performance.now() - start,
      };
  }
  ...
  if (!userPoolsResponse.UserPools || userPoolsResponse.UserPools.length === 0) {
      ...
      return {
          instructors: JSON.stringify([]), // Return empty list if no user pools are found
          executionDuration: performance.now() - start,
      };
  }
  ...
  if (deploymentType === "sandbox") {
      ...
      try {
          ...
          return {
              instructors: JSON.stringify([]), // Return empty list on error listing appsync apis
              executionDuration: performance.now() - start,
          };
      }
      ...
      if (!graphqlApisResponse.graphqlApis || graphqlApisResponse.graphqlApis.length === 0) {
          ...
          return {
              instructors: JSON.stringify([]), // Return empty list if no appsync apis are found
              executionDuration: performance.now() - start,
          };
      }
      ...
      if (!targetAppSync) {
          ...
          return {
              instructors: JSON.stringify([]), // Return empty list if no matching appsync instance
              executionDuration: performance.now() - start,
          };
      }
      ...
      if (!targetUserPoolId) {
          ...
          return {
              instructors: JSON.stringify([]), // Return empty list if user pool id is missing in appsync details
              executionDuration: performance.now() - start,
          };
      }
  ...
  if (!targetUserPoolId) {
      ...
      return {
          instructors: JSON.stringify([]), // Return empty list if no matching user pool is found
          executionDuration: performance.now() - start,
      };
  }
  ...
  // Step 4: List users in the "Instructor" group from the identified User Pool
  console.log("Step 4: Listing users in the 'Instructor' group from User Pool:", targetUserPoolId);
  try {
      const listUsersInGroupCommand = new ListUsersInGroupCommand({
          UserPoolId: targetUserPoolId,
          GroupName: 'Instructor', // Specify the group name
          Limit: 60, // Adjust as needed for pagination
      });

      const response = await cognitoClient.send(listUsersInGroupCommand);
      const users = response.Users || [];
      console.log(`Found ${users.length} users in the "Instructor" group in User Pool ${targetUserPoolId}.`);
      ...
      // Step 6: Return the list of instructors and execution duration
      console.log("Step 6: Returning instructor list and execution duration.");
      return {
          instructors: JSON.stringify(userDetails), // JSON-encoded list of instructor user details
          executionDuration: performance.now() - start,
      };
      ...
      return {
          instructors: JSON.stringify([]), // Return empty list on error fetching users
          executionDuration: performance.now() - start,
      };
  } finally {
      console.log("getInstructors Lambda function execution finished.");
  }
};