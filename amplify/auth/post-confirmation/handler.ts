...
export const handler: PostConfirmationTriggerHandler = async (event, context) => {
  try {
    // Step 1: Add user to the "Student" Cognito group
    ...
    try {
      ...
      console.log(`User ${username} added to Student Cognito group`);
    } catch (groupError) {
      console.error(`Error adding user to Student group: ${groupError}`);
      // Continue with DynamoDB update even if group assignment fails
    }

    // Step 2: Get the current Lambda function's tags
    ...
    // Step 3: Construct the DynamoDB table name
    const tableName = `User-${apiId}-NONE`;

    // Step 4: Insert a record into the DynamoDB table
